...
/**
 * Where a stored transcript's text came from.
 *   "captions" — YouTube's own subtitle track, fetched free via yt-dlp.
 *   "deepgram" — audio downloaded and paid for through Deepgram ASR.
 * Downstream features care: an auto-generated caption track has no
 * punctuation reliability and typical ASR word errors, an author-written
 * one is essentially ground truth, and Deepgram sits in between.
 */
export type TranscriptSource = "captions" | "deepgram";

export type TranscriptProvenance = {
  /** null on rows written before this column existed. */
  source: TranscriptSource | null;
  /**
   * 1 = the caption track was machine-generated by YouTube, 0 = written
   * by the video's author, null = unknown / not applicable (Deepgram).
   */
...
export function upsertTranscript(
  ...
  db.prepare(
    `INSERT INTO transcripts (video_id, language, text, source, auto_generated, fetched_at)
     VALUES (?, ?, ?, ?, ?, strftime('%s','now'))
    ...
    provenance.autoGenerated === undefined || provenance.autoGenerated === null
      ? null
      : provenance.autoGenerated
        ? 1
        : 0
  );
}
...
export function getTranscript(
  videoId: string
): ({ text: string; language: string | null } & TranscriptProvenance) | null {
  const row = db
    .prepare(
      "SELECT text, language, source, auto_generated FROM transcripts WHERE video_id = ?"
    )
    .get(videoId) as
    | ({ text: string; language: string | null } & TranscriptProvenance)
    | undefined;
  return row ?? null;
}

/**
 * Best guess at what language a channel's videos are spoken in, taken
 * from transcripts we have already stored for that channel. Used as a
 * hint when picking a subtitle track, so a German or Ukrainian channel
 * doesn't get an English track just because YouTube offers a machine
 * translation into English. Returns null when we have no evidence —
 * callers must NOT substitute a default.
 */
export function getChannelTranscriptLanguage(
  ...
  const row = db
    .prepare(
      `SELECT t.language AS language, COUNT(*) AS n
        ...
        WHERE v.channel_id = ? AND t.language IS NOT NULL AND t.language <> ''
        GROUP BY t.language
        ORDER BY n DESC
        LIMIT 1`
    )
    .get(channelId) as { language: string; n: number } | undefined;
...
export function upsertCompetitorTranscript(
  ...
  db.prepare(
    `INSERT INTO competitor_transcripts
       (competitor_id, video_id, language, text, source, auto_generated, fetched_at)
     VALUES (?, ?, ?, ?, ?, ?, strftime('%s','now'))
     ON CONFLICT(competitor_id, video_id) DO UPDATE SET
       language = excluded.language,
       text = excluded.text,
...
/**
 * True when we already stored a transcript for this competitor video.
 * Mirrors `hasCachedComments` in competitor-sync: caption text does not
 * change after upload, so a re-sync must not re-fetch (and re-risk the
 * YouTube rate limit) for videos already covered.
 */
export function hasCompetitorTranscript(
  competitorId: number,
  videoId: string
): boolean {
  return (
    db
      .prepare(
        `SELECT 1 FROM competitor_transcripts
          WHERE competitor_id = ? AND video_id = ? LIMIT 1`
      )
      .get(competitorId, videoId) !== undefined
  );
}

/**
 * Language most of this competitor's stored transcripts are in — the
 * same "use what we already know, never assume English" hint as
 * getChannelTranscriptLanguage, for competitor videos.
 */
export function getCompetitorTranscriptLanguage(
  competitorId: number
): string | null {
  const row = db
    .prepare(
      `SELECT language, COUNT(*) AS n
        ...
        WHERE competitor_id = ? AND language IS NOT NULL AND language <> ''
        GROUP BY language
        ORDER BY n DESC
        LIMIT 1`
    )
    .get(competitorId) as { language: string; n: number } | undefined;
  return row?.language ?? null;
}
...
/* ------------------------------------------------------------------
 * Transcript provenance columns.
 *
 * Added when free YouTube-caption transcripts landed alongside the paid
 * Deepgram path. Downstream features want to know how much to trust a
 * given transcript: an author-written caption track is essentially
 * ground truth, Deepgram ASR is good, and YouTube's machine captions
 * have no reliable punctuation and typical ASR word errors.
 *
 *   source          'captions' | 'deepgram' | NULL (written before this)
 *   auto_generated  1 = YouTube machine captions, 0 = author-written,
 *                   NULL = unknown / not applicable (Deepgram rows)
 *
 * Guarded PRAGMA + ALTER so existing databases keep working untouched;
 * pre-existing rows simply carry NULLs.
 * ------------------------------------------------------------------ */
{
  const provenanceColumns: { name: string; type: string }[] = [
    { name: "source", type: "TEXT" },
    { name: "auto_generated", type: "INTEGER" },
  ];
  for (const table of ["transcripts", "competitor_transcripts"]) {
    try {
      const existing = (
        db.prepare(`PRAGMA table_info(${table})`).all() as { name: string }[]
      ).map((c) => c.name);
      for (const col of provenanceColumns) {
        if (existing.includes(col.name)) continue;
        try {
          db.exec(`ALTER TABLE ${table} ADD COLUMN ${col.name} ${col.type}`);
        } catch (err) {
          // eslint-disable-next-line no-console
          console.warn(`[db] add ${table}.${col.name} failed (ignored):`, err);
        }
      }
    } catch {
      /* table not present yet — nothing to migrate */
    }
  }
}
